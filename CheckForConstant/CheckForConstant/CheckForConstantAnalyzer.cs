...
namespace CheckForConstant
{
    ...
    public class CheckForConstantAnalyzer : DiagnosticAnalyzer
    {
        ...
        private void AnalyzeNode(SyntaxNodeAnalysisContext context)
        {
            ...
            var dataFlowAnalisis = context.SemanticModel.AnalyzeDataFlow(localDeclaration);
            var variable = localDeclaration.Declaration.Variables.Single();
            var variableSimbol = context.SemanticModel.GetDeclaredSymbol(variable);

            if (dataFlowAnalisis.WrittenOutside.Contains(variableSimbol))
            {
                return;
            }
